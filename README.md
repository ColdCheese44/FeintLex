...
- **Click-to-capture (sentence mining)**: hover any word on a reading surface and **click it** to file it into your personal 🎯 Captured deck (`POST /tutor/capture`). Captured words automatically enter the drills (dedicated scope in the Drill tab), the Signal Queue, chat quizzes, and XP. The tooltip shows "🎯 tracked" for words you already captured. This is the Refold 1T-mining loop, automated.
- **File import**: the 📄 File button loads `.txt`, `.srt`, and `.vtt` files into Source Intake. Subtitle files are detected automatically on import — indices, timestamps, markup, and duplicate lines are stripped, leaving pure dialogue.
- **Writing prompts**: the Writing panel rotates through the PROGRAM.md SOP (summary, opinion, interrogation, prediction, retelling, free write) keyed to your active lesson; 🎲 for a different one.
- **Anki export**: the Anki button (Reinforcement panel) writes your full mastery + mistake bank to a tab-separated file in `exports/` (`POST /exports/anki`) — import into Anki with "Fields separated by: Tab".
- **Backups**: `.\scripts\run_cli.ps1 backup` zips a **hot copy** of the database (safe while the server runs) plus all exports into `backups/`.
- **Interleaved review**: the Signal Queue round-robins mistakes, sentence items, and weak terms instead of blocking by type — mixed practice retains better.
